...
pid_matches_service_binary() {
    local pid="${1:-}"
    local resolved_bin="" resolved_exe="" cmdline=""

    process_exists "${pid}" || return 1

    if command -v readlink >/dev/null 2>&1 && [ -e "${BIN_PATH}" ] && [ -e "/proc/${pid}/exe" ]; then
        resolved_bin=$(readlink -f "${BIN_PATH}" 2>/dev/null || true)
        resolved_exe=$(readlink -f "/proc/${pid}/exe" 2>/dev/null || true)
        if [ -n "${resolved_bin}" ] && [ -n "${resolved_exe}" ] && [ "${resolved_bin}" = "${resolved_exe}" ]; then
            return 0
        fi
    fi

    cmdline=$(ps -o args= -p "${pid}" 2>/dev/null | head -1)
    case "${cmdline}" in
        "${BIN_PATH}"*|*" ${BIN_PATH}"*)
            return 0
            ;;
...
openrc_service_pid() {
    ...
    if [ -f "${OPENRC_PIDFILE}" ]; then
        ...
        if [ -n "${pid}" ] && pid_matches_service_binary "${pid}"; then
            echo "${pid}"
            return 0
        fi
    fi
    ...
    while IFS= read -r candidate; do
        ...
        if pid_matches_service_binary "${candidate}"; then
            echo "${candidate}"
            return 0
        fi
    done < <(pgrep -x "$(basename "${BIN_PATH}")" 2>/dev/null)
...
parse_ps_etime_seconds() {
    ...
    raw=$(printf '%s' "${raw}" | xargs 2>/dev/null)
    [ -z "${raw}" ] && return 1
...
get_process_uptime() {
    local pid="$1"
    local uptime="" elapsed="" start_ticks="" uptime_secs="" clk_tck=""
    ...
    if [ -r /proc/uptime ] && [ -r "/proc/${pid}/stat" ]; then
        ...
        start_ticks=$(awk '{print $(NF-30)}' "/proc/${pid}/stat" 2>/dev/null)
    ...
    elapsed=$(ps -o etimes= -p "${pid}" 2>/dev/null | xargs)
    case "${elapsed}" in
        ''|*[!0-9]*)
            ;;
        *)
    ...
    if [ -n "${uptime}" ]; then
        elapsed=$(parse_ps_etime_seconds "${uptime}" 2>/dev/null) || return 1
        format_elapsed_seconds "${elapsed}"
        return $?
    fi
...
service_main_pid() {
    ...
    pid=$(systemctl show -p MainPID --value ${SERVICE_NAME} 2>/dev/null | xargs)
    ...
    if [ "${SERVICE_MANAGER}" = "openrc" ]; then
        pid=$(openrc_service_pid 2>/dev/null | xargs)
        if process_exists "${pid}"; then
            echo "${pid}"
            return 0
        fi
...
_node_user_remove_cert_overrides() {
    local NODE_ID="$1"
    local KEY
    for KEY in cert_file key_file cert_domain cert_mode cert_key_length acme_server dns_provider; do
        _node_user_delete_exact_key "${NODE_ID}" "${KEY}"
    done
    ...
    case ${CERT_CHOICE} in
        ...
        2)
            ...
            if [ -n "${S_CERT_DOMAIN}" ]; then
                ...
                read -p "ACME CA (letsencrypt/zerossl/自定义目录 URL) [letsencrypt]: " S_ACME_SERVER
                S_ACME_SERVER=${S_ACME_SERVER:-letsencrypt}
                if ! _validate_acme_server "${S_ACME_SERVER}"; then
                    echo -e "${RED}ACME CA 格式不合法: ${S_ACME_SERVER}${NC}"
                    return
                fi
                _node_user_remove_cert_overrides "${NODE_ID}"
                ...
                _node_user_set_key "${NODE_ID}" "acme_server" "${S_ACME_SERVER}"
        ...
        3)
            ...
            if [ -n "${S_CERT_DOMAIN}" ]; then
                ...
                if [ -z "${S_DNS_PROVIDER}" ]; then
                    ...
                    echo -e "${YELLOW}例如 Cloudflare: DNS_CF_Email=xxx@xx.com 和 DNS_CF_Key=xxxxx${NC}"
                    echo -e "${YELLOW}也兼容直接输入 CF_Email=... / CF_Key=...，脚本会自动转成 DNS_CF_*${NC}"
                    while true; do
                        read -p "  > " DNS_LINE
                        [ -z "${DNS_LINE}" ] && break
                        DNS_K="${DNS_LINE%%=*}"
...
do_status() {
    ...
    if service_is_active; then
        ...
        if [ -n "${PID}" ]; then
            print_process_runtime_summary "${PID}"
        fi
...
CERTEOF
                        echo -e "${YELLOW}请输入 DNS 环境变量（每行一个，格式: KEY=VALUE，空行结束）${NC}"
                        echo -e "${YELLOW}例如 Cloudflare: DNS_CF_Email=xxx@xx.com 然后 DNS_CF_Key=xxxxx${NC}"
                        echo -e "${YELLOW}也兼容直接输入 CF_Email=... / CF_Key=...，保存时会自动转成 DNS_CF_*${NC}"
                        while true; do
                            read -p "  > " DNS_LINE
                            [ -z "${DNS_LINE}" ] && break
                            DNS_K="${DNS_LINE%%=*}"
    ...
    case "${CERT_ACTION}" in
        auto)
            ...
            if [ -n "${NEW_CERT_DOMAIN}" ]; then
                ...
                read -p "  ACME CA (letsencrypt/zerossl/自定义目录 URL) [letsencrypt]: " NEW_ACME_SERVER
                NEW_ACME_SERVER=${NEW_ACME_SERVER:-letsencrypt}
                if ! _validate_acme_server "${NEW_ACME_SERVER}"; then
                    echo -e "${RED}  ACME CA 格式不合法: ${NEW_ACME_SERVER}${NC}"
                    return
                fi
                ...
                sed -i '/^acme_server=/d' "${CONFIG_FILE}"
...
_validate_acme_server() {
    local VALUE="$1"
    case "${VALUE}" in
        letsencrypt|zerossl|https://*) ;;
        *)
            return 1
            ;;
    esac
...
# 参数: domain key_length mode dns_provider dns_env_str acme_server
# dns_env_str 格式: "DNS_CF_Email=xxx DNS_CF_Key=yyy" 或 "CF_Email=xxx CF_Key=yyy"
_issue_cert_realtime() {
    ...
    if ! _validate_acme_server "${ACME_SERVER}"; then
        echo -e "${RED}ACME CA 格式不合法: ${ACME_SERVER}${NC}"
        return 1
    fi
    ...
    # 设置默认 CA 和注册账号
    echo -e "${CYAN}设置 ${ACME_SERVER} 为默认 ACME CA ...${NC}"
    HOME=/root "${ACME_BIN}" --set-default-ca --server "${ACME_SERVER}" >/dev/null 2>&1
    HOME=/root "${ACME_BIN}" --register-account -m cert-heki@gmail.com --server "${ACME_SERVER}" >/dev/null 2>&1
    ...
    # 构建 acme.sh --issue 命令
    local -a ISSUE_ARGS=(--issue -d "${DOMAIN}" --server "${ACME_SERVER}")

    if [ -n "${KEY_LEN}" ] && [ "${KEY_LEN}" != "RSA" ]; then
        ISSUE_ARGS+=(--keylength "${KEY_LEN}")
    fi
    ...
    if [ -n "${DNS_ENV_STR}" ]; then
        for pair in ${DNS_ENV_STR}; do
            ...
            DNS_ENV_V="${pair#*=}"
            case "${DNS_ENV_K}" in
                DNS_*) DNS_ENV_K="${DNS_ENV_K#DNS_}" ;;
            esac
            export "${DNS_ENV_K}=${DNS_ENV_V}"
        done
    fi
    ...
    # 执行申请，实时显示输出
    HOME=/root "${ACME_BIN}" "${ISSUE_ARGS[@]}" 2>&1
    local ISSUE_EXIT=$?
    ...
    local -a INSTALL_ARGS=(--install-cert -d "${DOMAIN}" --fullchain-file "${CERT_FILE}" --key-file "${KEY_FILE}")
    if echo "${KEY_LEN}" | grep -q "^ec-"; then
        INSTALL_ARGS+=(--ecc)
    fi

    HOME=/root "${ACME_BIN}" "${INSTALL_ARGS[@]}" 2>&1
    local INSTALL_EXIT=$?
...
_remove_old_cert_config() {
    ...
    sed -i '/^acme_server=/d' "${CONFIG_FILE}" 2>/dev/null
...
do_cert() {
    ...
    case ${cert_choice} in
        1)
            ...
            read -p "ACME CA [letsencrypt]: " ACME_SERVER
            ACME_SERVER=${ACME_SERVER:-letsencrypt}
            if ! _validate_acme_server "${ACME_SERVER}"; then
                echo -e "${RED}ACME CA 格式不合法: ${ACME_SERVER}${NC}"
                return
...
CERTEOF
            ...
            # 直接在脚本中申请证书，实时显示过程
            _issue_cert_realtime "${NEW_DOMAIN}" "${KEY_LEN}" "http" "" "" "${ACME_SERVER}"
            ;;
        2)
            read -p "证书域名: " NEW_DOMAIN
            [ -z "${NEW_DOMAIN}" ] && { echo -e "${RED}域名不能为空${NC}"; return; }
...
CERTEOF
            ...
            # 直接在脚本中申请证书，实时显示过程
            _issue_cert_realtime "${NEW_DOMAIN}" "${KEY_LEN}" "dns" "dns_cf" "CF_Email=${CF_EMAIL} CF_Key=${CF_KEY}" "${ACME_SERVER}"
            ;;
        3)
            read -p "证书域名: " NEW_DOMAIN
            [ -z "${NEW_DOMAIN}" ] && { echo -e "${RED}域名不能为空${NC}"; return; }
...
CERTEOF
            ...
            # 直接在脚本中申请证书，实时显示过程
            _issue_cert_realtime "${NEW_DOMAIN}" "${KEY_LEN}" "dns" "${DNS_PROV}" "${DNS_ENV_STR}" "${ACME_SERVER}"
            ;;
        4)
