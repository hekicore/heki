...
do_stop() {
    ...
    if [ ${rc} -eq 124 ]; then
        ...
        if systemctl is-active --quiet ${SERVICE_NAME}; then
            echo -e "${YELLOW}Heki 仍在运行，systemd 仍在等待旧进程退出；请查看: systemctl status ${SERVICE_NAME}${NC}"
        else
            echo -e "${GREEN}Heki 已停止${NC}"
        fi
        return
    ...
    echo -e "${RED}停止 Heki 失败，请查看日志: heki log${NC}"
}
...
do_modify() {
    ...
    echo -e "  面板类型: ${CUR_PANEL_TYPE} (1=sspanel-uim 2=metron 3=xboard 4=v2board 5=xiaov2board 6=ppanel)"
    ...
    if [ -n "${NEW_PANEL_TYPE_CHOICE}" ]; then
        case ${NEW_PANEL_TYPE_CHOICE} in
            ...
            sspanel-uim|metron|xboard|v2board|xiaov2board|ppanel) NEW_PANEL_TYPE="${NEW_PANEL_TYPE_CHOICE}" ;;
...
do_node() {
    ...
    case "${ACTION}" in
        ...
        *)
            ...
            echo "  heki node add 100    添加节点 100（协议自动识别，并按需引导证书/Reality 配置）"
