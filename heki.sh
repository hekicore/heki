...
do_modify() {
    ...
    echo -e "  面板类型: ${CUR_PANEL_TYPE} (1=sspanel-uim 2=xboard 3=v2board 4=xiaov2board 5=ppanel)"
    ...
    if [ -n "${NEW_PANEL_TYPE_CHOICE}" ]; then
        case ${NEW_PANEL_TYPE_CHOICE} in
            ...
            5) NEW_PANEL_TYPE="ppanel" ;;
            sspanel-uim|xboard|v2board|xiaov2board|ppanel) NEW_PANEL_TYPE="${NEW_PANEL_TYPE_CHOICE}" ;;
...
do_node() {
    ...
    case "${ACTION}" in
        add)
            ...
            # 询问节点类型（支持不同节点使用不同协议）
            local GLOBAL_SERVER_TYPE
            GLOBAL_SERVER_TYPE=$(grep "^server_type=" "${CONFIG_FILE}" 2>/dev/null | head -1 | cut -d= -f2-)
            echo ""
            echo -e "${CYAN}选择该节点的协议类型:${NC}"
            echo -e "  ${GREEN}0.${NC} 与主配置相同 (${GREEN}${GLOBAL_SERVER_TYPE}${NC})（默认）"
            echo -e "  ${GREEN}1.${NC} v2ray / vmess"
            echo -e "  ${GREEN}2.${NC} vless"
            echo -e "  ${GREEN}3.${NC} ss (Shadowsocks)"
            echo -e "  ${GREEN}4.${NC} ssr (ShadowsocksR)"
            echo -e "  ${GREEN}5.${NC} trojan"
            ...
            echo -e "  ${GREEN}10.${NC} tuic (TUIC V5)"
            echo ""
            read -p "选择 [0]: " ADD_NODE_TYPE_CHOICE
            ADD_NODE_TYPE_CHOICE=${ADD_NODE_TYPE_CHOICE:-0}
            local ADD_NODE_TYPE=""
            case ${ADD_NODE_TYPE_CHOICE} in
                1) ADD_NODE_TYPE="v2ray" ;;
                2) ADD_NODE_TYPE="vless" ;;
                3) ADD_NODE_TYPE="ss" ;;
                4) ADD_NODE_TYPE="ssr" ;;
                5) ADD_NODE_TYPE="trojan" ;;
                6) ADD_NODE_TYPE="hysteria" ;;
                7) ADD_NODE_TYPE="anytls" ;;
                8) ADD_NODE_TYPE="naive" ;;
                9) ADD_NODE_TYPE="mieru" ;;
                10) ADD_NODE_TYPE="tuic" ;;
                0|"") ADD_NODE_TYPE="" ;;
                *) echo -e "${RED}无效选项，使用主配置类型${NC}"; ADD_NODE_TYPE="" ;;
            esac
            ...
            # 如果选择了不同的协议类型，写入节点独立配置文件的 [USER] 区
            if [ -n "${ADD_NODE_TYPE}" ] && [ "${ADD_NODE_TYPE}" != "${GLOBAL_SERVER_TYPE}" ]; then
                local NODE_CONF_DIR="${INSTALL_DIR}/nodes"
                local NODE_CONF_FILE="${NODE_CONF_DIR}/node_${TARGET_ID}.conf"
                mkdir -p "${NODE_CONF_DIR}"
                if [ -f "${NODE_CONF_FILE}" ]; then
                    # 文件已存在（之前启动过的节点）
                    # 检查 [USER] 区是否已有 server_type
                    local HAS_USER_ST=false
                    local IN_USER_SEC=false
                    while IFS= read -r line; do
                        case "${line}" in *"[USER]"*) IN_USER_SEC=true ;; esac
                        if ${IN_USER_SEC} && echo "${line}" | grep -q "^server_type="; then
                            HAS_USER_ST=true
                            break
                        fi
                    done < "${NODE_CONF_FILE}"
                    ...
                    if ${HAS_USER_ST}; then
                        # [USER] 区已有 server_type，用 awk 只替换 [USER] 区的那行
                        awk -v newtype="${ADD_NODE_TYPE}" '
                            /\[USER\]/ { in_user=1 }
                            in_user && /^server_type=/ { $0="server_type=" newtype; done_replace=1 }
                            { print }
                        ' "${NODE_CONF_FILE}" > "${NODE_CONF_FILE}.tmp" && mv "${NODE_CONF_FILE}.tmp" "${NODE_CONF_FILE}"
                    else
                        # [USER] 区没有 server_type
                        if grep -q "\[USER\]" "${NODE_CONF_FILE}" 2>/dev/null; then
                            # 有 [USER] 区，追加到末尾
                            echo "server_type=${ADD_NODE_TYPE}" >> "${NODE_CONF_FILE}"
                        else
                            # 没有 [USER] 区，创建
                            echo "" >> "${NODE_CONF_FILE}"
                            echo "# ---- [USER] 用户自定义（修改这里，重启保留）----" >> "${NODE_CONF_FILE}"
                            echo "server_type=${ADD_NODE_TYPE}" >> "${NODE_CONF_FILE}"
                        fi
                    fi
                else
                    # 文件不存在（新节点，还没启动过），创建只含 [USER] 区的配置
                    cat > "${NODE_CONF_FILE}" << NODEEOF
# ---- [USER] 用户自定义（修改这里，重启保留）----
server_type=${ADD_NODE_TYPE}
NODEEOF
                fi
                echo -e "${GREEN}已为节点 ${TARGET_ID} 设置独立协议类型: ${ADD_NODE_TYPE}${NC}"
            elif [ -n "${ADD_NODE_TYPE}" ]; then
                echo -e "${YELLOW}节点 ${TARGET_ID} 使用与主配置相同的协议类型: ${ADD_NODE_TYPE}${NC}"
            fi

            # 检查是否已配置 Reality 私钥，如果没有则提示
            local EFFECTIVE_TYPE="${ADD_NODE_TYPE:-${GLOBAL_SERVER_TYPE}}"
            if [ "${EFFECTIVE_TYPE}" = "vless" ]; then
        ...
        list|ls|"")
            ...
            if [ -d "${NODES_DIR}" ]; then
                ...
                if [ -n "${NODE_FILES}" ]; then
                    ...
                    for nf in ${NODE_FILES}; do
                        ...
                        # 检查是否有 [USER] 区的 server_type 覆盖
                        local USER_TYPE_OVERRIDE=""
                        local IN_USER=false
                        while IFS= read -r cfgline; do
                            ...
                            if ${IN_USER}; then
                                case "${cfgline}" in
                                    server_type=*) USER_TYPE_OVERRIDE="${cfgline#server_type=}" ;;
                                esac
                            fi
                        done < "${nf}"
                        local TYPE_NOTE=""
                        ...
                        NORMALIZED_TYPE=$(normalize_server_type_alias "${NTYPE}")
                        NORMALIZED_OVERRIDE=$(normalize_server_type_alias "${USER_TYPE_OVERRIDE}")
                        [ -n "${USER_TYPE_OVERRIDE}" ] && [ "${NORMALIZED_OVERRIDE}" != "${NORMALIZED_TYPE}" ] && TYPE_NOTE=" ${YELLOW}(override→${USER_TYPE_OVERRIDE}, 待重启)${NC}"
                        echo -e "  Node ${NID}: ${GREEN}${NTYPE}${NC} | port=${NPORT} | ${DETAIL} ${STATUS_MARK}${TYPE_NOTE}"
                    done
            ...
            for cid in "${CONFIGURED_IDS[@]}"; do
                ...
                if ! ${HAS_RUNNING_CONF}; then
                    # 检查是否有预配置的 server_type（用户在 add 时选择了不同类型）
                    local PENDING_TYPE="${GLOBAL_TYPE}"
                    if [ -f "${CONF_FILE}" ]; then
                        local PRE_TYPE
                        PRE_TYPE=$(grep "^server_type=" "${CONF_FILE}" 2>/dev/null | head -1 | cut -d= -f2-)
                    ...
                    echo -e "  Node ${cid}: ${YELLOW}待启动${NC} (类型=${PENDING_TYPE}) ${YELLOW}[需重启]${NC}"
                fi
            done
            ;;
        *)
            ...
            echo "  heki node add 100    添加节点 100（支持选择不同协议类型）"
...
show_menu() {
    ...
    echo -e "  ${GREEN}14.${NC} 更新 Heki（正式版）"
    echo -e "  ${GREEN}15.${NC} 更新 Heki（测试版）"
    echo -e "  ${GREEN}16.${NC} 卸载 Heki"
    echo -e "  ${GREEN}17.${NC} 查看版本"
...
case "$1" in
    ...
    help|-h|--help)
        ...
        echo "  node add <ID>      添加节点（支持选择不同协议类型）"
