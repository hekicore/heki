...
install_heki() {
    ...
    if ${INSTALL_BETA}; then
        ...
        beta_url="https://github.com/${GITHUB_REPO}/releases/download/${beta_tag}/heki-linux-${arch}.tar.gz"
        echo -e "最新测试版: ${beta_tag}"
        echo -e "下载地址: ${beta_url}"
        wget -N --no-check-certificate -O /usr/local/heki.tar.gz "${beta_url}"
        if [[ $? -ne 0 ]]; then
